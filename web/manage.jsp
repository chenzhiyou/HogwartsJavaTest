...
<body>
    ...
    <script>
                            ...
                            "<div class=\"movie-item\">" +
                                "<a href=\"javascript:void(0)\">" +
                                    "<div class=\"movie-poster\">" +
                                        "<img src=\""+ obj.data[i].moviePicture +"\" onclick=\"movieDetail("+obj.data[i].movieId+")\">" +
                                        "<div class=\"movie-overlay movie-overlay-bg\">" +
                                            "<div class=\"movie-info\">" +
                                                "<div class=\"movie-score\"><i class=\"integer\">"+ obj.data[i].movieScore +"</i></div>" +
                                                "<div class=\"movie-title movie-title-padding\" title=\"\">"+ obj.data[i].movieCnName +"</div>\"" +
                                            "</div>" +
                                        "</div>" +
                                    "</div>" +
                                "</a>" +
                    ...
                    ,yes: function(){
                        ...
                        if(flag == 0){
                            if(file == null){
                                ...
                                formData.append("movieCnName",movie_cn_name);
                                formData.append("movieFgName",movie_fg_name);
                                ...
                                formData.append("movieReleaseDate",movie_releaseDate);
                        ...
                        if(flag == 1){
                            ...
                            $.ajax({
                                ...
                                success:function (obj) {
                                    ...
                                    $('#movie_cn_name').val(obj.data.movieCnName);
                                    ...
                                    $('#movie_fg_name').val(obj.data.movieFgName);
                                    $('#movie_releaseDate').val(obj.data.movieReleaseDate);
                                    $('#movie_country').val(obj.data.movieCountry);
                                    $('#demo1').attr('src', obj.data.moviePicture);
        ...
        //             ,title: '场次列表'
        //             ,cols: [[
        //                 {field:'scheduleId', title:'ID', width:102, unresize: true, sort: true}
        //                 ,{field:'cinemaName', title:'电影院', width:270, unresize: true,sort: true,templet: '<div>{{data.scheduleHall.hallCinema.cinemaName}}</div>'}
        //                 ,{field:'hallName', title:'放映厅', width:270, unresize: true, sort: true, edit: "text",templet: '<div>{{data.scheduleHall.scheduleHall.hallName}}</div>'}
        //                 ,{field:'scheduleStartTime', title:'放映时间', width:270, unresize: true, sort: true, edit: "text"}
        //                 ,{field:'schedulePrice', title:'售价', width:270, unresize: true, edit: "text"}
        ...
        //             var data = obj.data;
        //             if(obj.event === 'detail'){
        //                 layer.msg('ID：'+ data.scheduleId + '</br>电影院：'+ data.scheduleHall.hallCinema.cinemaName + '</br>放映厅：'+ data.scheduleHall.hallName  + '</br>放映时间：'+ data.scheduleStartTime + '</br>售价：' + data.schedulePrice, {offset: clientHeight/5});
        //             }
        //             else if(obj.event === 'edit'){
        ...
        //                                 schedule_cinema: data.scheduleHall.hallCinema.cinemaName,
        //                                 schedule_hall: data.scheduleHall.hallName,
        //                                 schedule_startTime: data.scheduleStartTime,
        //                                 schedule_price: data.schedulePrice,
        //                             },
        //                             success:function (date) {
        //                                 if(date == "success"){
        //                                     layer.alert('修改成功！',{icon: 0,offset: clientHeight/5},
            ...
            layui.use('table', function(){
                ...
                table.render({
                    ...
                    ,cols: [[
                        ...
                        ,{field:'commentUser.userName', title:'用户账号', width:100, unresize: true,templet: '<div>{{d.commentUser.userName}}</div>'}
            ...
            layui.use('table', function(){
                ...
                table.render({
                    ...
                    ,cols: [[
                        ...
                        ,{field:'orderUser.userName', title:'用户账号', width:100, unresize: true,templet:'<div>{{d.orderUser.userName}}</div>'}
                        ,{field:'order_schedule', title:'场次', width:180, unresize: true,templet:'<div>{{d.orderSchedule.scheduleStartTime}}</div>'}
                        ,{field:'orderPosition',title:'位置',width:100,unresize:true}
                        ,{field:'order_schedule',title:'价格',width:80,unresize:true,templet:'<div>￥{{d.orderSchedule.schedulePrice}}</div>'}
                        ,{field:'order_schedule',title:'电影',width:240,unresize:true,templet:'<div>{{d.orderSchedule.scheduleMovie.movieCnName}}</div>'}
                        ,{field:'order_schedule', title:'影厅', width:100, unresize: true, templet:'<div>{{d.orderSchedule.scheduleHall.hallName}}</div>'}
                        ,{field:'order_schedule',title:'影院',width:240,unresize:true,templet:'<div>{{d.orderSchedule.scheduleHall.hallCinema.cinemaName}}</div>'}
                        ,{field:'orderState',title:'订单状态',width:100,unresize:true,align:'center',templet:function(d){if(d.orderState == 1) return '<div style="color:#337ab7">完成</div>';else if(d.orderState == 0) return '<div style="color:#ef4238">申请退票</div>';else return '<div style="color:#5cb85c">已退票</div>';}}
                        ,{title:'操作', width:150, unresize: true, align:'center', toolbar: '#backticketbar'}
                    ]]
                    ,page: {layout: ['limit', 'count', 'prev', 'page', 'next', 'skip'] //自定义分页布局
                        ,curr: 1 //设定初始在第 1 页
                        ,groups: 5 //只显示 5 个连续页码
                ...
                table.on('tool(TicketTable)', function(obj){
                    var data = obj.data;
                    if(obj.event === 'detail'){
                        layer.msg('订单编号：'+ data.orderId + '&nbsp;&nbsp;&nbsp;用户：' + data.orderUser.userName + '<br>场次：'+ data.orderSchedule.scheduleStartTime + '&nbsp;&nbsp;&nbsp;位置：' + data.orderPosition + '<br>电影：《' + data.orderSchedule.scheduleMovie.movieCnName + '》&nbsp;&nbsp;&nbsp;价格：￥' + data.orderSchedule.schedulePrice + '<br>影院：'+ data.orderSchedule.scheduleHall.hallCinema.cinemaName + '&nbsp;&nbsp;&nbsp;影厅：' + data.orderSchedule.scheduleHall.hallName, {offset: clientHeight/4,area:['400px','140px']});
                    }
            ...
            layui.use(['table','form'], function(){
                ...
                table.render({
                    ...
                    ,cols: [[
                        ...
                        ,{field:'schedule_hall', title:'影院', width:240, unresize: true,templet:'<div>{{d.scheduleHall.hallCinema.cinemaName}}</div>'}
                        ,{field:'schedule_hall', title:'影院地址', width:300, unresize: true,templet:'<div>{{d.scheduleHall.hallCinema.cinemaAddress}}</div>'}
                        ,{field:'schedule_hall', title:'影厅', width:100, unresize: true, templet:'<div>{{d.scheduleHall.hallName}}</div>'}
                        ,{field:'schedule_movie', title:'电影', width:240, unresize: true,templet:'<div>{{d.scheduleMovie.movieCnName}}</div>'}
                        ,{field:'scheduleStartTime', title:'放映时间', width:180, unresize: true, sort: true}
                        ,{field:'schedulePrice', title:'价格(￥)', width:100,align:'center', unresize: true}
                        ,{field:'orderList',title:'订单数量',width:100,unresize:true,align:'center',templet:'<div>{{d.orderList.length}}</div>'}
                        ,{field:'scheduleRemain',title:'剩余票数',width:100,align:'center',unresize:true}
                        ,{field:'orderList',title:'场次收入(￥)',width:120,align:'center',unresize:true,templet:'<div>{{d.orderList.length * d.schedulePrice}}</div>'}
                ...
                table.on('tool(ScheduleTable)', function(obj){
                    ...
                    if(obj.event === 'detail'){
                        console.log(obj);
                        layer.msg('编号：'+ data.scheduleId + '</br>影院：'+ data.scheduleHall.hallCinema.cinemaName + '&nbsp;&nbsp;&nbsp;放映厅：'+ data.scheduleHall.hallName  + '</br>影院地址：'+ data.scheduleHall.hallCinema.cinemaAddress + '</br>电影：'+ data.scheduleMovie.movieCnName + '</br>场次：' + data.scheduleStartTime + '&nbsp;&nbsp;&nbsp;价格：' + data.schedulePrice, {offset: clientHeight/5});
                    }
                    else if(obj.event === 'edit'){
                                            ...
                                            for(var i = 0;i < cinemaArr.length;i++){
                                                ...
                                                for(var key in cinemaArr[i]){
                                                    ...
                                                    for(var j = 0;j < cinemaArr[i][key].length; j++){
                                                        hallArr.push(cinemaArr[i][key][j].hallName);
                                                    }
            ...
            $.ajax({
                ...
                success:function (obj) {
                    ...
                    for(var i = obj.sort.length - 1;i >= 0;i--){
                        movieArray.push(obj.sort[i].movieCnName);
                        boxOffice.push(Math.ceil(obj.sort[i].movieBoxOffice * 10000));
                    }

    ...
    <script>
        ...
        function initInformation(){
            ...
            $.ajax({
                ...
                success:function (obj) {
                    ...
                    for(var i=0;i<TempLength;i++){//用数组记录座位的行和列
                        Seatrow[i] = obj.data.orderList[i].orderPosition.split("排")[0];
                        Seatcol[i] = obj.data.orderList[i].orderPosition.split("排")[1].split("座")[0];
                    }
                    console.log(obj);
                    PriceTemp = obj.data.schedulePrice;
                    movieInfo.append(
                        "<div class=\"poster\">" +
                            "<img src=\"" + obj.data.scheduleMovie.moviePicture + "\">" +
                        "</div>" +
                        "<div class=\"content\">" +
                            "<p class=\"name text-ellipsis\">" + obj.data.scheduleMovie.movieCnName + "</p>" +
                            "<div class=\"info-item\">" +
                                "<span>类型：</span>" +
                                "<span class=\"value\">" + obj.data.scheduleMovie.movieType + "</span>" +
                            "</div>" +
                            "<div class=\"info-item\">" +
                                "<span>时长：</span>" +
                                "<span class=\"value\">" + obj.data.scheduleMovie.movieDuration + "</span>" +
                            "</div>" +
                        "</div>"
                    );
                    showInfo.append(
                        ...
                        "<div class=\"info-item\">" +
                            "<span>版本：</span>" +
                            "<span class=\"value\">" +  obj.data.scheduleMovie.movieCountry + "</span>" +
                        "</div>" +
                        "<div class=\"info-item\">" +
                            "<span>场次：</span>" +
        ...
        function initSeat(){
            ...
            for(var i=0;i<12;i++)
            {
                ...
                for(var j=0;j<12;j++)
                {
                    if(TempLength==0){//倘若这个场次的订单为0，则走这个逻辑
